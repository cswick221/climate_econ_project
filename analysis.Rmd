...
```{r}
opinion_qs <- c("geoid", "geoname", "year", "affectweather", "discuss",  "fundrenewables", "futuregen",  "governor", "happening", "harmus", "personal", "rebates", "regulate", "worried") # all of the opinion questions that are relevant imo, removed a couple that didn't have any data from before 2021
```
...
for (i in 1:length(indicators)) {
  ...
  dind_df_subset <- dind_df_subset %>% 
    mutate(lhs := get(indicators[i]))
  model <- lm(lhs ~ in_out + time + in_out * time, data = dind_df_subset)
  summary(model)
...
# Transform your data into a pdata.frame
pdata_df <- pdata.frame(dind_df_subset, index = c(id_var, time_var))

# Fit a fixed effects model
fixed_effects_model <- plm(worried ~ in_out + time + in_out * time, data = pdata_df, model = "within")

# Print the fixed effects model summary
summary(fixed_effects_model)
```

### Fixed effects regression loop
...
fe_results_by_indicator <- data.frame()
...
for (i in 1:length(indicators)) {
  ...
  pdata_df <- pdata.frame(dind_df_subset, index = c(id_var, time_var))
  # Fit a fixed effects model
  fe_model <- plm(lhs ~ in_out + time + in_out * time, data = pdata_df, model = "within")
  summary(fe_model)
  
 tidy_fe_results <- tidy(fe_model) %>%
    mutate(indicator = indicators[i])
  
  # Bind the results to the main data frame
  fe_results_by_indicator <- bind_rows(fe_results_by_indicator, tidy_fe_results)
}

fe_did_regression_results <- fe_results_by_indicator %>% 
  filter(term == "skylight_treatment:pre_post_treatment") %>% 
  janitor::clean_names() %>% 
  rename(regression_estimate = estimate)
